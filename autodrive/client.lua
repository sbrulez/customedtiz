...
-- give feedback when reversing and drive the car automatically in the speed
-- range of 5-40mph.
-- Here's you find only client side functions and server side call's
------
            ...
            if DoesEntityExist(vehicle) then                                           
                ...
                if IsControlJustPressed(1, Config.SportModeKey) then                
                    if sportmode then
                        if Config.EnableSportMode then
                            exports['mythic_notify']:DoHudText('inform', 'Sports Mode Disabled!', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
                        end
                    else                                    
                        if Config.EnableSportMode then
                            sportmode = true
                            exports['mythic_notify']:SendAlert('inform', 'Sports Mode Enabled!', 5000)
                        end
                    end            
                end
                ...
                if lights > 0 and highbeam < 1 and IsControlJustPressed(1, 74) then
                    ...
                    if road ~= 0 then
                        ...
                        exports['mythic_notify']:SendAlert('inform', 'The Reflector Would Blind The Incoming Traffic!', 5000)
                    end
                ...
                if Config.EnableAutodrive then
                    if IsControlJustPressed(1, Config.AutodriveKeycode) and autodrive == false then
                        if IsWaypointActive() then                              
                            ...
                            exports['mythic_notify']:SendAlert('inform', 'Auto-Pilot Enabled!', 5000)
                            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'on', Config.BeepSoundVolume) 
                            autodrive = true                                             
                        else
                            exports['mythic_notify']:SendAlert('inform', 'Please Select The Destination!', 5000)
                    ...
                    if IsControlJustPressed(1, s_key) and autodrive or IsControlJustPressed(1, w_key) and autodrive or IsControlJustPressed(1, space) and autodrive or IsControlJustPressed(1, a_key) and autodrive or IsControlJustPressed(1, d_key) and autodrive then
                        ...
                        exports['mythic_notify']:SendAlert('inform', 'Auto Drive Disabled!', 5000)
                    ...
                    if autodrive then
                        ...
                        if destinationlength < 10 then                                                
                            if speed > 1 then
                                ...
                                ClearPedTasks(PlayerPedId())
                                exports['mythic_notify']:SendAlert('inform', 'AutoDrive Disabled!', 5000)
                                TriggerServerEvent('InteractSound_SV:PlayOnSource', 'off', Config.BeepSoundVolume) 
                                autodrive = false
                            end
                        end
                        if IsControlJustPressed(1, Config.AutodriveDecreaseSpeedKey) then
                            if cruisespeed > Config.AutodriveMinSpeed then
                                ...
                                exports['mythic_notify']:SendAlert('inform', 'You Have Reached The Min. Speed Limit For Auto Drive!', 5000)
                            end
                        elseif IsControlJustPressed(1, Config.AutodriveIncreaseSpeedKey) then
                            if cruisespeed < Config.AutodriveMaxSpeed then
                                cruisespeed = cruisespeed + 2.5
                            else
                                exports['mythic_notify']:SendAlert('inform', 'You Have Reached The Max. Speed Limit For Auto Drive!', 5000)
                            end
                        end
                    end                
                end
